      ...
      Post
        ...
        .then(post => {
          if (post.likes.filter(like => 
            ...
            return res.status(400).json({alreadyLike: 'You already liked this post'})
          }
    ...
    Post
      ...
      .then(post => {
        if (post.likes.filter(like => 
          ...
          return res.status(400).json({ notLiked: 'You have not yet this post' })
        }
  ...
  Post
    ...
    .then(post => {
      if (post.comments.filter(comment => comment.id === req.params.comment_id).length === 0) {
        return res.status(404).json({commentNotExisting: 'This comment does not exists'})
      }
...
// @desc    Comment a post
// @access  Private
router.post('/comment/:id', passport.authenticate('jwt', {session: false}), commentPost);

// @route   DELETE api/posts/uncomment/:id/:comment_id
// @desc    Uncomment a post
// @access  Private
router.delete('/uncomment/:id/:comment_id', passport.authenticate('jwt', {session: false}), uncommentPost);
